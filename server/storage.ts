...
export const storage = {
  ...
  async getCampaignByUrl(uniqueUrl: string): Promise<Campaign | undefined> {
    const result = await db.select().from(campaigns).where(eq(campaigns.uniqueUrl, uniqueUrl));
    return result[0];
  },

  async createCampaign(data: InsertCampaign): Promise<Campaign> {
    const uniqueUrl = `${data.name.toLowerCase().replace(/\s+/g, '-')}-${nanoid(8)}`;
  ...
  async getSubmissions(): Promise<(Submission & { campaignName: string })[]> {
    ...
    }).from(submissions).leftJoin(campaigns, eq(submissions.campaignId, campaigns.id));
  ...
  async getSubmissionsByCampaign(campaignId: number): Promise<Submission[]> {
    return await db.select().from(submissions).where(eq(submissions.campaignId, campaignId));
  },
  ...
  async updateSubmissionStatus(id: number, status: string): Promise<Submission | undefined> {
    const result = await db.update(submissions).set({ status }).where(eq(submissions.id, id)).returning();
    return result[0];
  },
  ...
  async createCustomersBulk(data: InsertCustomer[]): Promise<Customer[]> {
    const result = await db.insert(customers).values(data).returning();
    return result;
  },
